      ...
      <el-card class="login-card">
          ...
          <div>
            ...
            <span class="text-sm flex items-center">
              ...
              <span class="ml-1" @click="agreed = true">
                我已阅读并同意
                <span style="color: #3370ff" class="cursor-pointer" @click="handleAgreementClick">
                  《用户协议》
                </span>
              </span>
            </span>
        ...
        <main v-show="step === 2" class="wh-full relative">
          ...
          <div class="mt-4 mb-2">
            ...
            <p class="text-sm">
              请输入发送至
              <span class="font-bold text-bold">+86{{ maskedPhone }}</span>
              的 6 位验证码，5 分钟内有效。
            </p>
          </div>
          ...
          <div class="code-inputs-wrapper">
            ...
            <div class="code-group">
              <el-input
                ...
                :key="index + 2"
                ref="codeInputs"
                v-model="code[index + 2]"
                :class="getInputClass(index + 2)"
                type="text"
                inputmode="numeric"
                maxlength="1"
                autocomplete="off"
                size="large"
                @input="handleInput(index + 2, $event)"
                @keydown="handleKeydown(index + 2, $event)"
                @focus="handleFocus(index + 2)"
                @blur="handleBlur"
                @paste="handlePaste"
              />
            </div>
          ...
          <p class="mb-2">
            ...
            <span v-else class="cursor-pointer text-blue-500" @click="debouncedResendCode">
              重新获取验证码
            </span>
          </p>
          ...
          <el-button
            type="primary"
            class="w-full mb-4 absolute bottom-0"
            size="large"
        ...
        <main v-show="step === 3" class="wh-full flex flex-col relative">
          ...
          <div class="step-title mt-4">你可以进入一下组织</div>
          <div class="text-sm">
            <span class="font-bold text-bold">+86{{ maskedPhone }}</span>
            已在以下组织绑定了账号，你可以进入以下任意组织
          </div>
